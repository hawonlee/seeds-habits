...
export const HabitCard = ({
          ...
          <div className="mt-3">
            <div className="relative w-full h-1.5 bg-gray-200 rounded-full">
              <div
                ...
                style={{ width: `${adoptionProgressPct}%` }}
              />
            </div>
            <div className="mt-1 text-right text-xs text-muted-foreground">
              {habit.streak}/{adoptionThreshold} days
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-gray-200">
        <CardContent className="p-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium truncate max-w-[12rem]">{habit.title}</span>
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-muted-foreground">{habit.target_frequency}x/wk</span>
              <Checkbox
                checked={done}
                onCheckedChange={(next) => {
                  const shouldCheck = Boolean(next);
                  if (shouldCheck) {
                    if (onCheckInForDate) onCheckInForDate(habit.id, targetDate);
                    else if (onCheckIn) onCheckIn(habit.id);
                  } else {
                    if (onUndoCheckInForDate) onUndoCheckInForDate(habit.id, targetDate);
          ...
          {/* Weekly completion overview (7 boxes) */}
          <div className="mt-2 flex gap-1">
            {Array.from({ length: 7 }, (_, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`w-6 h-6 rounded border flex items-center justify-center text-[10px] ${dayDone ? 'text-white' : ''}`}
  ...
  return (
    <div className="relative" ref={cardRef}>
      <Card 
        className="mb-4 group relative cursor-pointer hover:shadow-md transition-shadow"
        onClick={() => setShowInlineEdit(true)}
      >
      {/* Plus button for future habits */}
      {habit.phase === 'future' && (
      ...
      <CardContent>
        ...
        <div className="mt-4">
          <div className="relative w-full h-2 bg-gray-200 rounded-full">
            <div
              className="absolute left-0 top-0 h-2 bg-black rounded-full transition-all duration-300"
              style={{ width: `${adoptionProgressPct}%` }}
            />
          </div>
          <div className="mt-1 text-right text-xs text-muted-foreground">
