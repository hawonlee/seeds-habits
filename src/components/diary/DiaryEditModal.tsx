...
export const DiaryEditModal: React.FC<DiaryEditModalProps> = ({
  ...
  // Immediate save helper to guarantee the picked date is persisted
  ...
  const handleInput = (e: React.FormEvent<HTMLDivElement>) => {
    ...
    setBody(newBody.replace(/<br\s*\/?>/gi, '\n').replace(/<div>/gi, '\n').replace(/<\/div>/gi, ''));
  ...
  const handleClose = () => {
    // If user hasn't typed anything, just close without saving
    if (!hasUserTyped) {
      onClose();
      return;
    }
    
    // If user has typed something, save before closing
    flushDebounce();
    onClose();
  };
